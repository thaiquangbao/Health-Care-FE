...
const HoSo = () => {
    ...
    useEffect(() => {
        const width1 = document.querySelector(`.item-1`).offsetWidth
        const width2 = document.querySelector(`.item-2`).offsetWidth
        const width3 = document.querySelector(`.item-3`).offsetWidth
        let width = 0
        let remainWidth = 0
        if (choose === 1) {
            width = width1
    ...
    return (
        <div className="w-full flex flex-col pt-[1%] pb-[3%] px-[5%] background-public">
            <Navbar />
            <input accept='.png, .jpg, .jpeg' onChange={() => handleUpdateAvatar()} ref={imgRef} type='file' className='hidden' />
            <div className='flex flex-col gap-4 mt-4'>
                <div className='w-full relative flex justify-around items-end'>
                    <div className='flex'>
                        <div className='aspect-square w-[12%] relative flex items-center justify-start'>
                            ...
                            <div style={{ backgroundImage: `url(${userData.user?.image})`, backgroundSize: 'cover' }} className='rounded-full w-[800px] aspect-square' />
                            <div onClick={() => imgRef.current.click()} className='h-[30px] flex justify-center items-center rounded-full w-[30px] text-[22px] absolute right-[0px] bg-[#f0f0f0] cursor-pointer bottom-[5px]'><i className='text-[#9a9a9a] bx bx-camera'></i></div>
                        </div>
                        <div className='flex flex-col justify-center ml-[2rem] translate-y-[15px]'>
                            <div><span className='text-[22px] font-semibold'>{userData.user?.fullName}</span></div>
                            <span className='text-[14px] font-medium text-[#5f5f5f]'>{userData.user?.phone}</span>
                        </div>
                    </div>
                    {choose !== 2 ? (
                        <>{visibleUpdate ? <button onClick={() => handleUpdateUser()} className='text-[white] bg-[blue] w-[200px] py-2 rounded-md font-medium text-[14px]'>Cập Nhật</button> : <div className='w-[200px]'></div>}</>
                    )
                        :
                ...
                <div className='w-full flex flex-col ml-10 items-start gap-1'>
                    <ul ref={listRef} className='flex text-[14px] font-medium gap-[1rem]'>
                        <li onClick={() => setChoose(1)} className='item-1 px-2 cursor-pointer'>Thông Tin Cá Nhân</li>
                        <li onClick={() => setChoose(2)} className='item-2 px-2 cursor-pointer'>Bảo Mật</li>
                        <li onClick={() => setChoose(3)} className='item-3 px-2 cursor-pointer'>Thanh Toán</li>
                    </ul>

...
import { convertDateToDayMonthYearTimeObject, convertDateToDayMonthYearVietNam, editNumber } from "@/utils/date";
...
const FormRecordPatientHome = ({ type, setType, setTemporary, doctorRecord1, appointmentHome1, setReload }) => {
  ...
  useEffect(() => {
    if (currentMedicalRecord) {
      ...
      setDiagnosisDisease(currentMedicalRecord.diagnosisDisease)
      setSymptoms(currentMedicalRecord.symptoms)
      setNote(currentMedicalRecord.note)
      setReAppointmentDate(`${currentMedicalRecord.reExaminationDate.year}-${editNumber(currentMedicalRecord.reExaminationDate.month)}-${editNumber(currentMedicalRecord.reExaminationDate.day)}`)
      setTemperature(currentMedicalRecord.temperature)
      setBloodPressure(currentMedicalRecord.bloodPressure)
      setHealthRate(currentMedicalRecord.healthRate)
      setWeight(currentMedicalRecord.weight)
      setHeight(currentMedicalRecord.height)
    }
  ...
  useEffect(() => {
    ...
    if (appointmentHome1) {
      api({ type: TypeHTTP.GET, sendToken: false, path: `/medicalRecords/get-appointment/${appointmentHome1._id}` })
        .then(res => {
  ...
  return (
    <div className="min-w-[100%] h-full flex">
      <div style={{ marginLeft: `${screen * -100}%`, transition: '0.5s' }} className="w-[100%] h-full flex">
        <div className="px-[2rem] min-w-[100%] overflow-auto h-full py-[1rem] flex flex-col gap-2">

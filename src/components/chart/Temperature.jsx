...
export default function Temperature({ logBook, setLogBook }) {
  ...
  useEffect(() => {
    if (chartRef.current && logBook) {
      ...
      const times = logBook.disMon
        .filter((item) => item.vitalSign.temperature !== 0)
        .map(
          (item) =>
            `(${item.date.time}) ${item.date.day}/${item.date.month}/${item.date.year}`
        )
        .slice(-10);
      const temperatures = logBook.disMon
        .filter((item) => item.vitalSign.temperature !== 0)
        .map((item) => item.vitalSign.temperature)
        .slice(-10);
  ...
  const handleSubmit = () => {
    ...
    api({
      sendToken: false,
      type: TypeHTTP.POST,
      path: "/chats/temperature-warning",
      body: dataAI,
    }).then((resAI) => {
      authHandler.showHealthResponse({
        message: `Nhiệt độ cơ thể ngày hôm nay của bạn: ${resAI.comment} ${resAI.advice}`,
      });
      const body = {
        _id: logBook._id,
        disMonItem: {
      ...
      api({
        type: TypeHTTP.POST,
        sendToken: true,
        path: "/healthLogBooks/update-temperature",
        body,
      }).then((res) => {
        setLogBook(res);
        api({

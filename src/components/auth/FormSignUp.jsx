...
const FormSignUp = ({ visible, hidden }) => {
        ...
        if (info.phone !== '' && currentStep === 2) {
            setTimeout(() => {
                ...
                if (recaptchaContainer) {
                    ...
                    signInWithPhoneNumber(auth, formatPhoneByFireBase(info.phone), recaptcha)
                        .then(confirmation => {
    ...
    const handleSubmitSignUp = () => {
        ...
        api({ sendToken: false, type: TypeHTTP.POST, body: { phone: info.phone, passWord: info.password }, path: '/auth/signup' })
            .then(res => {
                ...
                globalHandler.notify(notifyType.SUCCESS, 'Đăng Ký Tài Khoản Thành Công')
            })
            .catch(error => {
        ...
        verification.confirm(otp)
            .then(data => {
                ...
                api({ type: TypeHTTP.POST, body: { ...user }, path: `/auth/update`, sendToken: false })
                    ...
                    .catch(() => {
                        globalHandler.notify(notifyType.FAIL, 'Xác minh thất bại, Vui lòng thử lại')
                    })
            })
            .catch(() => {
    ...
    const handleCompleteInfo = () => {
        ...
        if (!individual.dateOfBirth || new Date().getFullYear() - new Date(individual.dateOfBirth).getFullYear() - (new Date().getMonth() < new Date(individual.dateOfBirth).getMonth() || (new Date().getMonth() === new Date(individual.dateOfBirth).getMonth() && new Date().getDate() < new Date(individual.dateOfBirth).getDate())) < 12) {
        ...
        globalHandler.notify(notifyType.LOADING, 'Đang hoàn thành thông tin cá nhân')
        let user = {
            ...userData.user,
            processSignup: 3,
            ...
            .then(res => {
                api({ type: TypeHTTP.POST, body: { _id: res._id }, path: `/auth/generateToken`, sendToken: false })
                    .then(token => {
                        ...
                        setCurrentStep(4)
                        userHandler.setUser(res)
                        globalHandler.notify(notifyType.SUCCESS, 'Đã Hoàn Thành Thông Tin Tài Khoản')
                    })
            })
    }

    return (
        <div style={visible ? { height: 'auto', width: '70%', transition: '0.3s' } : { height: 0, width: 0, transition: '0.3s' }} className='z-50 w-[70%] min-h-[100px] flex items-center overflow-hidden bg-[white] rounded-lg fixed top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]'>
            {visible && (
                <div style={{ transition: '0.5s', marginLeft: `-${(currentStep - 1) * 100}%` }} className='w-[100%] flex'>
                    <div className='min-w-[100%] h-full px-[2rem] py-[3rem] flex justify-center'>
                        ...
                        <div className='w-[53%] flex flex-col items-start justify-center pl-[3rem] gap-3'>
                            <h2 className='text-[20px] font-medium mb-1'>Đăng Ký Tài Khoản Với HealthHaven</h2>
                            <input onChange={e => setInfo({ ...info, phone: e.target.value })} value={info.phone} placeholder='Số Điện Thoại (+84)' className='text-[14px] w-[90%] h-[40px] bg-[white] border-[1px] border-[#cfcfcf] focus:outline-0 rounded-lg px-4' />
                            <input type='password' onChange={e => setInfo({ ...info, password: e.target.value })} value={info.password} placeholder='Mật Khẩu' className='text-[14px] w-[90%] h-[40px] bg-[white] border-[1px] border-[#cfcfcf] focus:outline-0 rounded-lg px-4' />
                            <input type='password' onChange={e => setInfo({ ...info, confirmPassword: e.target.value })} value={info.confirmPassword} placeholder='Xác Nhận Mật Khẩu' className='text-[14px] w-[90%] h-[40px] bg-[white] border-[1px] border-[#cfcfcf] focus:outline-0 rounded-lg px-4' />
                            <button onClick={() => handleSubmitSignUp()} className='hover:scale-[1.05] transition-all text-[14px] bg-[blue] px-[3rem] w-[270px] text-[white] h-[37px] rounded-lg'>Đăng Ký Tài Khoản</button>
                            ...
                            }} className='text-[14px] hover:underline'>Bạn đã có tài khoản?</button>
                        </div>
                    </div>
                    <div className='min-w-[100%] h-full px-[2rem] py-[3rem] flex justify-center'>
                        <img src='/sign.png' width={'40%'} />
                        <div className='w-[53%] flex flex-col items-start justify-center pl-[3rem] gap-1'>
                            <h2 className='text-[20px] font-medium '>Xác Thực Tài Khoản Với Số Điện Thoại</h2>
                    ...
                    <div className='min-w-[100%] h-full px-[2rem] py-[3rem] flex justify-center'>
                        <img src='/sign.png' width={'40%'} />
                        <div className='w-[53%] flex flex-col items-start justify-center pl-[3rem] gap-1'>
                            <h2 className='text-[20px] font-medium '>Hoàn Thành Thông Tin Cá Nhân</h2>
                            <span className='text-[13px]'>Hãy bổ sung thông tin cá nhân của bạn ở bên dưới.</span>
                            <input onChange={e => setIndividual({ ...individual, name: e.target.value })} placeholder='Họ Và Tên' className='text-[13px] mt-1 w-[90%] h-[38px] bg-[white] border-[1px] border-[#cfcfcf] focus:outline-0 rounded-lg px-4' />
                            <input type='date' onChange={e => setIndividual({ ...individual, dateOfBirth: e.target.value })} placeholder='Ngày Sinh' className='text-[13px] mt-1 w-[90%] h-[38px] bg-[white] border-[1px] border-[#cfcfcf] focus:outline-0 rounded-lg px-4' />
                            <select onChange={e => setIndividual({ ...individual, sex: Boolean(e.target.value) })} className='text-[13px] mt-1 w-[90%] h-[38px] bg-[white] border-[1px] border-[#cfcfcf] focus:outline-0 rounded-lg px-3'>
                                <option value={null}>Giới Tính</option>
                                <option value={true}>Nam</option>
                                <option value={false}>Nữ</option>
                            </select>
                            <input onChange={e => setIndividual({ ...individual, cccd: e.target.value })} placeholder='Căn Cước Công Dân' className='text-[13px] mt-1 w-[90%] h-[38px] bg-[white] border-[1px] border-[#cfcfcf] focus:outline-0 rounded-lg px-4' />
                            <input onChange={e => setIndividual({ ...individual, address: e.target.value })} placeholder='Địa Chỉ' className='text-[13px] mt-1 w-[90%] h-[38px] bg-[white] border-[1px] border-[#cfcfcf] focus:outline-0 rounded-lg px-4' />
                            <button onClick={() => handleCompleteInfo()} className='hover:scale-[1.05] transition-all text-[14px] bg-[blue] px-[3rem] w-[270px] text-[white] mt-2 h-[37px] rounded-lg'>Hoàn Thành</button>
                        </div>
                    </div>

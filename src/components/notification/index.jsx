...
const Notification = ({ status, message, setInfomation }) => {
    ...
    return (
        <div style={{ right: info.status === 'none' ? '-500px' : '8px', transition: '0.3s', backgroundColor: info.status === notifyType.SUCCESS ? 'white' : info.status === notifyType.FAIL ? 'red' : info.status === notifyType.LOADING ? 'white' : 'yellow', color: info.status === notifyType.SUCCESS ? 'black' : info.status === notifyType.FAIL ? 'white' : info.status === notifyType.LOADING ? 'black' : '#6b6b6b' }} className='items-center px-3 flex w-[270px] shadow-xl z-50 rounded-md min-h-[70px] fixed top-4'>
            ...
            <i style={{ color: (info.status === notifyType.SUCCESS || info.status === notifyType.LOADING) ? '#999' : 'white' }} onClick={() => setInfo({ status: 'none', message: '' })} className='bx bx-x absolute top-1 right-1 text-[21px] cursor-pointer' ></i>
        </div>
    )
}

...
import { compare2Date, convertDateToDayMonthYear, convertDateToMinuteHour } from '@/utils/date'
      ...
      api({
        ...
        .then((res) => {
          ...
          const filter = res.schedules.filter(item => {
            return compare2Date(item.date, bookingData.booking.appointment_date)
          })[0]
          const nestedFilter = filter.times.filter(item => item.time === bookingData.booking.appointment_date.time)[0]
          if (nestedFilter.status !== '') {
            globalHandler.notify(notifyType.WARNING, "Cập nhật: Thời gian hẹn đã được đặt, vui lòng chọn thời gian khác")
            setTimeout(() => {

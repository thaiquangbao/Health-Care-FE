...
const FormBenhNhanDetail = ({ logBookUpdate, onClose, visibleTransfer, setVisibleTransfer, visibleMedicalRecord, setVisibleMedicalRecord }) => {
  ...
  useEffect(() => {
    ...
    if (logBookUpdate) {
      setRoom(authData.rooms.filter(item => (item.doctor._id === logBookUpdate.doctor._id && item.patient._id === logBookUpdate.patient._id))[0])
    }
  }, [logBookUpdate])
  ...
  return (
    <section
      ref={itemRef}
      style={{
        height: (logBook && (visibleTransfer === false && visibleMedicalRecord === false)) ? "90%" : 0,
        width: (logBook && (visibleTransfer === false && visibleMedicalRecord === false)) ? "90%" : 0,
        transition: "0.4s",
    ...
    >
      {(logBook && (visibleTransfer === false && visibleMedicalRecord === false)) && (
        <div style={{ marginLeft: `${-screen * 50}%`, transition: '0.5s' }} className="flex w-[100%]">
          ...
          <div className="px-[2rem] py-[1.5rem] min-w-[100%] flex flex-col">
            <span style={{ color: logBook.status.status_type !== 'STOPPED' ? 'black' : 'red' }} className="font-semibold text-[25px]">
              Thông tin cá nhân ({logBook.status.status_type !== 'STOPPED' ? 'Đang theo dõi sức khỏe' : "Đã dừng theo dõi sức khỏe"})
            </span>
              ...
              <div className="flex flex-col w-[30%]">
                ...
                <div className="flex flex-col py-2 gap-1 w-[100%]">
                  {logBook.status.status_type !== 'STOPPED' && (
                    <>
                      <button className="py-2 rounded-full bg-gradient-to-r from-blue-400 to-blue-600 text-white" onClick={() => {
                        setVisibleTransfer(true)
                      }}>Chuyển bác sĩ</button>
                      <button className="py-2 rounded-full bg-gradient-to-r from-purple-400 to-purple-600 text-white" onClick={() => { setVisibleMedicalRecord(true) }}>Xem hồ sơ sức khỏe</button>
                    </>
                  )}

                  <button onClick={() => setScreen(2)} className="py-2 rounded-full bg-gradient-to-r from-orange-400 to-orange-600 text-white">{logBook.status.status_type !== 'STOPPED' ? 'Nhắc nhở bệnh nhân' : 'Gửi tin nhắn'}</button>
                </div>
              </div>
          ...
          <div className="p-[0.5rem] min-w-[100%] flex flex-col items-center">
            <Chat room={room} setRoom={setRoom} setScreen={setScreen} />
          </div>
        </div>
      )}
    </section>
